...
class Helper:
    ...
    def __init__(self, params):
        #print(**params)
        self.params = Params(**params)#获取所有参数

        self.times = {'backward': list(), 'forward': list(), 'step': list(),
                      'scales': list(), 'total': list()}
        if self.params.random_seed is not None:
            self.fix_random(self.params.random_seed)

        self.make_folders() #配置日志功能
        self.make_task() #下载数据集
        # self.make_synthesizer() #生成中毒文件
        self.attack = Attack(self.params) #攻击

        # if 'neural_cleanse' in self.params.loss_tasks: #保留了neural_cleanse功能
        #     self.nc = True
    ...
    def make_folders(self):
        ...
        if self.params.log:
            ...
            with open('saved_models/runs.html', 'a') as f:
                f.writelines([f'<div><span> <a href="http://gpu/'
